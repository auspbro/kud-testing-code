...
def _subprocess_Popen(*args, **kwargs):
    ...
    process = subprocess.Popen(*_get_subprocess_args(args), **kwargs)
...
class AndroidDevice(object):
    ...
    def read(self, count=1):
        ...
        if (self.command_result):
            result = self.command_result[0:count]
            self.command_result = self.command_result[count:]
        return result
        
    def write(self, cmd):
        self.command_result = ''
        cmd = split_lines(cmd)
        self.command_result = split_lines(self.shell(cmd)[0])[0]
        return self.command_result
        
    def close(self):
        self.sync()
        self.shell(['exit'])
        return self._simple_call(['kill-server'])
        
    def tcp_connect(self):
        host = ''
        
        wlan_info = split_lines(self.shell(['netcfg | grep wlan'])[0])[0]
#        print wlan_info
        pattern = re.compile(ur'\b\d+(?:\.\d+){3}\b', re.MULTILINE)
        ip = re.findall(pattern, wlan_info)[0]
        if ip == '0.0.0.0':
            print('>> FAIL: WiFi is disabled, please enable WiFi before running this script <<')
            return host

        port = '5555'
        result = split_lines(self._simple_call(['tcpip', port]))[0]
        if result.find('restarting in TCP mode port') == -1:
            self.tcp_disconnect()
            print('>> FAIL: Please unplug and replug the USB cable <<')
            return host
        
        host = "%s:%s" %(ip, port)
        result = split_lines(self._simple_call(['connect', host]))[0]
        if result.find('connected to') == -1:
            self.tcp_disconnect()
            print('>> FAIL: Please check the domain name is correct <<')
            return host

        adb_list = get_devices()
        if host in adb_list:
            print('>> PASS: connect to %s (%s) <<' % (host, self.serial))
        else:
            host = ''
            self.tcp_disconnect()
            print('>> FAIL: cannot connect to %s (%s) <<' % (host, self.serial))

        return host
        
    def tcp_disconnect(self):
        result = split_lines(self._simple_call(['usb']))[0]
        if result.find('restarting in USB mode') == -1:
            print('>> FAIL: restarting in USB mode <<')
        else:
            print('>> PASS: restarting in USB mode <<')
            print('>> Please unplug and replug the USB cable <<')

    # ------------------ same seriral format: end ------------------
...
def main():
    ...
    if len(get_devices()) > 0:
        serial_sn = '0123456789ABCDEF'
        debug = AndroidDevice(serial=serial_sn)
        print debug.get_prop('sys.usb.vid')
    else:
